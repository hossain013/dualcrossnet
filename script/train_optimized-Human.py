...
# ──────────────────────────────────────────────────────────────
SCRIPT_DIR = '../ppi/data/'
H5_PATH  = os.path.join(SCRIPT_DIR, "LR_PPI-ProtT5-Full.h5")
CSV_PATH = os.path.join(SCRIPT_DIR, "LR_PPI.csv")
...
max_seq = 1500
df = df[(df['Protein_1_Seq'].str.len()<=max_seq) & (df['Protein_2_Seq'].str.len()<=max_seq)]
...
class DataSequenceLoader(Sequence):
    def __init__(self, df, batch_size=32, shuffle=True,
        ...
        self.x1_emb    = df["Protein_1_Seq"].values
        self.x2_emb    = df["Protein_2_Seq"].values
