"""
Ablation Study: ESM2 + LLaMA Self-Attention ONLY (No Cross-Attention)
======================================================================
Change from full model:
  - REMOVED: bidirectional cross-attention blocks (c1, c2)
  - KEPT:    two LLaMA self-attention blocks (s1, s2), one per protein
  - KEPT:    residual + dropout, hybrid pooling, MLP head — all unchanged
  - KEPT:    all training settings (focal loss, AdamW, warmup-cosine LR, etc.)

Purpose: measure contribution of cross-attention to overall performance.
"""
...
# ──────────────────────────────────────────────────────────────
# CONFIG  (identical to full model — only architecture changes)
# ──────────────────────────────────────────────────────────────
...
CONFIG = {
    ...
    "ablation"        : "no_cross_attention",   # ← ablation tag
}

PROJECT_NAME = f"{CONFIG['dataset']}-ABLATION-NoCrossAttn-{CONFIG['architecture']}-{TIMESTAMP}"
...
print(f"[INFO] ABLATION: No Cross-Attention")
...
class WarmupCosineDecay(tf.keras.optimizers.schedules.LearningRateSchedule):
    ...
    def __call__(self, step):
        ...
        warmup_lr = self.peak_lr * step / tf.maximum(w_steps, 1.0)
...
# ──────────────────────────────────────────────────────────────
# 6. Model — ABLATION: LLaMA self-attention only, NO cross-attention
# ──────────────────────────────────────────────────────────────
...
def build_model(drop_pool=0.30, drop_linear=0.20, bn_momentum=0.90,
    ...
    # LLaMA self-attention — one block per protein (UNCHANGED)
    s1 = llama_self_attention(p1, mask1, conv_out, heads)
    s2 = llama_self_attention(p2, mask2, conv_out, heads)

    # ── ABLATION: cross-attention removed ──
    # Full model had:
    #   c1 = cross_attention_block(p1, p2, p2, mask2, ...)
    #   c2 = cross_attention_block(p2, p1, p1, mask1, ...)
    #   sc1 = Dropout(Add([s1, c1]))
    #   sc2 = Dropout(Add([s2, c2]))
    # Ablation: use s1/s2 directly with dropout
    sc1 = layers.Dropout(drop_pool)(s1)
    sc2 = layers.Dropout(drop_pool)(s2)

    # Hybrid pooling (unchanged)
...
for fold, (train_idx, valid_idx) in enumerate(skf.split(df, df["Interaction"]), 1):
    ...
    try:
        ...
        run = wandb.init(
            ...
            group="KFold-CV-ABLATION-NoCrossAttn",
