...
1.  AdamW optimizer  (weight_decay=1e-2) — better regularisation
2.  Cosine-annealing LR with linear warmup  (peak=3e-4, warmup=10%)
3.  Binary Focal Loss  (γ=2.0, α=0.5) + label smoothing=0.05
    → focuses training on hard/uncertain examples
4.  Gradient clipping  (clipnorm=1.0) — stable with higher LR
5.  Reduced dropout  (pool=0.30, linear=0.20) — less underfitting in 3 epochs
6.  Checkpoint on val_auc  (not val_loss) — directly optimises ranking metric
7.  Dual threshold evaluation:
      A) threshold=0.50 — SAME as SOTA RLEAAI (github.com/zhouyu9931/RLEAAI)
         for a fair apples-to-apples comparison
      B) Youden's J optimal threshold — additional gain on top
8.  BatchNorm momentum  0.95 → 0.90 — faster running-stat adaptation
...
# Disable W&B interactive prompt; runs offline if no key set
...
# ──────────────────────────────────────────────────────────────
SCRIPT_DIR = 'data/Sars-Cov-19/'
H5_PATH  = os.path.join(SCRIPT_DIR, "sars_esmt36_Full.h5")
CSV_PATH = os.path.join(SCRIPT_DIR, "dataset_SARS-CoV-2.xlsx")
...
CONFIG = {
    ...
    "random_state"    : SEED,
    # ── training schedule
    "epochs"          : 60,          # quick 3-epoch run per fold
    "peak_lr"         : 3e-4,       # ↑ 6× higher than baseline 5e-5
    "min_lr"          : 1e-6,
    "warmup_ratio"    : 0.10,       # 10 % of total steps for linear warmup
    # ── optimiser
    "optimizer"       : "AdamW",
    "weight_decay"    : 1e-2,       # L2 via AdamW
    ...
    # ── model head (unchanged architecture)
    "drop_pool"       : 0.30,       # was 0.50
    "drop_linear"     : 0.20,       # was 0.30
    "bn_momentum"     : 0.90,       # was 0.95
    # ── meta
...
PROJECT_NAME = f"{CONFIG['dataset']}-OPT-{CONFIG['architecture']}-{TIMESTAMP}"
...
# ──────────────────────────────────────────────────────────────
print("[INFO] Loading ESM embeddings from HDF5 …")
load_seq = {}
...
def binary_focal_loss(gamma=2.0, alpha=0.5, label_smoothing=0.05):
    ...
    - gamma : focusing exponent (0 = vanilla BCE)
    - alpha : class balance weight for positive class
    - label_smoothing : softens hard labels to reduce overconfidence
    """
...
class DataSequenceLoader(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i in batch_idx:
            e1 = load_seq[self.x1_emb[i]].reshape(-1, 2560)
            e2 = load_seq[self.x2_emb[i]].reshape(-1, 2560)
...
def build_model(drop_pool=0.30, drop_linear=0.20, bn_momentum=0.90,
    ...
    inp1  = Input((None, 2560));  inp2  = Input((None, 2560))
    mask1 = Input((None,));       mask2 = Input((None,))

    x_dim = 384
    # Stem: project ESM dim → 384
    p1 = layers.Dense(x_dim, use_bias=False, name="stem1")(inp1)
    p1 = layers.BatchNormalization(momentum=bn_momentum, name="bn1")(p1)
    p2 = layers.Dense(x_dim, use_bias=False, name="stem2")(inp2)
    p2 = layers.BatchNormalization(momentum=bn_momentum, name="bn2")(p2)
    ...
    # Residual + dropout
    sc1 = layers.Dropout(drop_pool)(layers.Add()([s1, c1]))
    sc2 = layers.Dropout(drop_pool)(layers.Add()([s2, c2]))

    # Hybrid pooling
    merged = layers.Concatenate()([HybridPooling()(sc1), HybridPooling()(sc2)])
...
for fold, (train_idx, valid_idx) in enumerate(skf.split(df, df["label"]), 1):
    ...
    callbacks = [
        ...
        tf.keras.callbacks.ModelCheckpoint(
            ...
            save_best_only  = True,
            monitor         = "val_auc",   # ← optimise AUC directly
            mode            = "max",
        ),
    ...
    # ─────────────────────────────────────────────
    # Evaluation A: threshold = 0.5  (same as SOTA RLEAAI paper)
    ...
    # Maximises sensitivity + specificity jointly.
    # ─────────────────────────────────────────────
